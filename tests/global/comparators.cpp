...
auto main() -> int {
  {
    ...
    raise::ErrorIf(ulp_eq(f, f1) != cmp::AlmostEqual(f, f1),
                   "Wrong comparison with ULP-1 #f1",
                   HERE);
    raise::ErrorIf(ulp_eq(f, f1, 2) != cmp::AlmostEqual(f, f1, 1e-6f),
...

  {
    ...
    raise::ErrorIf(ulp_eq(f1, f2) && cmp::AlmostEqual(f1, f2, 1e-8f),
                   ...
                   HERE);
    raise::ErrorIf(ulp_eq(f1, std::nexttoward(f1, +INFINITY)) &&
...

  {
    ...
    auto   f1     = a * a / 23.0;
    auto   f2     = a / b;
    raise::ErrorIf((not ulp_eq(f1, f2)) || (not cmp::AlmostEqual(f1, f2)),
                   "Wrong comparison with ULP-1 #d1",
                   HERE);
    raise::ErrorIf(not cmp::AlmostZero((f1 - f2) * (f2 - f1)),
