...
auto measure_Dflux(ntt::Meshblock<ntt::Dim2, ntt::GRPICEngine>& mblock,
...

    {
      ...
      while (sim.time() < 15.0) {
        ...
        if (sim.time() > 13) {
          ...
          if (fluxes_set && (!ntt::AlmostEqual(flux1_prev, flux_1, diff) ||
                             !ntt::AlmostEqual(flux2_prev, flux_2, diff))) {
            throw std::runtime_error(
              "int D is not constant (t > 13): " + std::to_string(flux_1 * 1e7) +
        ...
        if (sim.time() < 3.5) {
          ...
          if (!ntt::CloseToZero(flux_1, tiny)) {
            printf("flux: %e\n", flux_1);
            throw std::runtime_error(
              "int D at r1 is not zero (t < 4): " + std::to_string(flux_1));
          }
        }
      }
    }
