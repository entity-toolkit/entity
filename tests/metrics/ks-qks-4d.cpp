...
template <Dimension D>
Inline void unravel(size_t                    idx,
                    tuple_t<size_t, D>&       ijk,
                    const tuple_t<size_t, D>& res) {
  for (auto d { 0u }; d < D; ++d) {
    ijk[d]  = idx % res[d];
    idx    /= res[d];
  }
 ...
 */
template <class M>
void testGInverse(const std::vector<ncells_t>&         res,
  ...
  Kokkos::parallel_for(
    ...
    Lambda(cellidx_t n) {
      ...
      if (not almostZero(od01, scale, acc) || not almostZero(od03, scale, acc) ||
          not almostZero(od10, scale, acc) || not almostZero(od13, scale, acc) ||
          not almostZero(od30, scale, acc) || not almostZero(od31, scale, acc)) {
 ...
 */
template <class M>
void testGSpatial(const std::vector<ncells_t>&         res,
                  const boundaries_t<real_t>&          ext,
                  const real_t                         acc,
                  const std::map<std::string, real_t>& params) {
  static_assert(M::Dim == 2, "Dim != 2");
  ...
  Kokkos::parallel_for(
    ...
    Lambda(cellidx_t n) {
      ...
      vec_t<Dim::_4D> g_ij { metric.template g_<1, 1>(x_Code),
                             metric.template g_<2, 2>(x_Code),
                             metric.template g_<3, 3>(x_Code),
                             metric.template g_<1, 3>(x_Code) };
      vec_t<Dim::_4D> h_ij_expect { metric.template h_<1, 1>(x_Code),
 ...
 */
template <class M>
void testG00Analytical(const std::vector<ncells_t>&         res,
                       const boundaries_t<real_t>&          ext,
                       const real_t                         acc,
                       const std::map<std::string, real_t>& params) {
  static_assert(M::Dim == 2, "Dim != 2");
 ...
 */
template <class M>
void testU0(const std::vector<ncells_t>&         res,
            const boundaries_t<real_t>&          ext,
            const real_t                         acc,
            const std::map<std::string, real_t>& params) {
  static_assert(M::Dim == 2, "Dim != 2");
  ...
  Kokkos::parallel_for(
    ...
    Lambda(cellidx_t n) {
      ...
      // Test with non-zero spatial velocity
      {
        ...
        const auto contraction = g00 * SQR(u0) + TWO * g01 * u0 * u_i[0] +
                                 g11 * SQR(u_i[0]) + g22 * SQR(u_i[1]) +
                                 g33 * SQR(u_i[2]) + TWO * g13 * u_i[0] * u_i[2];
 ...
 */
template <class M>
void testTransform4d(const std::vector<ncells_t>&         res,
                     const boundaries_t<real_t>&          ext,
                     const real_t                         acc,
                     const std::map<std::string, real_t>& params) {
  static_assert(M::Dim == 2, "Dim != 2");
  ...
  Kokkos::parallel_for(
    ...
    Lambda(cellidx_t n) {
      ...
      // Verify transform_4d<D,U> is consistent with direct g^{mu nu} multiplication
      {
        ...
        vec_t<Dim::_4D> v_cntrv_direct { g00 * v_cov[0] + g01 * v_cov[1],
                                         g01 * v_cov[0] + g11 * v_cov[1] +
                                           g13 * v_cov[3],
                                         g22 * v_cov[2],
...
void runAllTests(const char*                          label,
                 const std::vector<ncells_t>&         res,
                 const boundaries_t<real_t>&          ext,
                 const real_t                         acc,
                 const std::map<std::string, real_t>& params,
                 bool                                 has_kerr_g00 = false) {
  std::cout << "[" << label << "] testGInverse..." << '\n';
  testGInverse<M>(res, ext, acc, params);
  std::cout << "  passed." << '\n';

  std::cout << "[" << label << "] testGSpatial..." << '\n';
  testGSpatial<M>(res, ext, acc, params);
  std::cout << "  passed." << '\n';

  if (has_kerr_g00) {
    std::cout << "[" << label << "] testG00Analytical..." << '\n';
    testG00Analytical<M>(res, ext, acc, params);
    std::cout << "  passed." << '\n';
  }

  std::cout << "[" << label << "] testU0..." << '\n';
  testU0<M>(res, ext, acc, params);
  std::cout << "  passed." << '\n';

  std::cout << "[" << label << "] testTransform4d..." << '\n';
...
auto main(int argc, char* argv[]) -> int {
  ...
  try {
    ...
    const auto res = std::vector<ncells_t> { 32, 42 };
    ...
    std::cout << "=== QKerrSchild (a=0.8, h=0) ===" << '\n';
    const auto qks_h0 = std::map<std::string, real_t> {
      { "r0",        -TWO },
      {  "h",        ZERO },
      {  "a", (real_t)0.8 }
    };
    runAllTests<QKerrSchild<Dim::_2D>>("QKS h=0", res, ext, acc, qks_h0, true);

    // ---- QKerrSchild (h=0.25, with angular stretching) ----
    std::cout << "=== QKerrSchild (a=0.8, h=0.25) ===" << '\n';
    const auto qks_h025 = std::map<std::string, real_t> {
      { "r0",         -TWO },
      {  "h", (real_t)0.25 },
      {  "a",  (real_t)0.8 }
    };
    runAllTests<QKerrSchild<Dim::_2D>>("QKS h=0.25", res, ext, acc, qks_h025, true);

    // ---- QKerrSchild (low spin) ----
    std::cout << "=== QKerrSchild (a=0.3, h=0.1) ===" << '\n';
    const auto qks_low = std::map<std::string, real_t> {
      { "r0",        -TWO },
      {  "h", (real_t)0.1 },
      {  "a", (real_t)0.3 }
    };
    runAllTests<QKerrSchild<Dim::_2D>>("QKS a=0.3", res, ext, acc, qks_low, true);
