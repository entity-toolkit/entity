...

    {
      ...
      for (auto n { 0 }; n < nmax + 1; ++n) {
...

        {
          ...
          if (n == nmax) {
            !(ntt::CloseToZero(SQR(dist), (real_t)1e-4)) ? throw std::logic_error(fmt::format(
              "particle not in init position: {:.6e} != 0.0, L2 = {:.6e}", dist, SQR(dist)))
                                                         : (void)0;
            !(ntt::AlmostEqual(maxdist, TWO * larmor, (real_t)1e-3)) ? throw std::logic_error(
              fmt::format("maxdist is incorrect: {:.6f} != {:.6f}", maxdist, TWO * larmor))
                                                                     : (void)0;
...

        {
          ...
          !(ntt::CloseToZero(upar)) ? throw std::logic_error(
