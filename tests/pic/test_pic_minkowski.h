  ...
  SUBCASE("Minkowski") {
    ...
    SUBCASE("2D pusher [E x B]") {
      ...
      try {
        ...
        CHECK(AlmostEqual(sim.mblock()->timestep(), 0.041984464973f));
        CHECK(AlmostEqual(sim.sim_params()->sigma0(), 0.01f));

        real_t beta_drift = 0.1f;

        // initialize Bz
        {
          auto mblock = sim.m_mblock;
          Kokkos::parallel_for(
            "set fields",
        ...
        {
          auto electrons = sim.m_mblock.particles[0];
          auto positrons = sim.m_mblock.particles[1];
          auto mblock    = sim.m_mblock;
          Kokkos::parallel_for(
          ...
          electrons.set_npart(1);
          positrons.set_npart(1);
        }

        sim.synchronizeHostDevice();

        for (int i {0}; i < 2; ++i) {
          CHECK(sim.m_mblock.particles[i].i1_h(0) == 33);
          CHECK(sim.m_mblock.particles[i].i2_h(0) == 33);
          CHECK(AlmostEqual(sim.m_mblock.particles[i].dx1_h(0), 0.6f));
          CHECK(AlmostEqual(sim.m_mblock.particles[i].dx2_h(0), 0.92f));
        }
        ...
        sim.mblock()->metric.x_Code2Cart(
          {(real_t)(sim.mblock()->particles[0].i1_h(0)) + sim.mblock()->particles[0].dx1_h(0),
           (real_t)(sim.mblock()->particles[0].i2_h(0)) + sim.mblock()->particles[0].dx2_h(0)},
          x_p);
        std::cout << "x_p1 = " << sim.mblock()->particles[0].i1_h(0)
                  << ", dx_p1 = " << sim.mblock()->particles[0].dx1_h(0) << std::endl;
        std::cout << "x_p[0] = " << x_p[0] << ", x_p1_fin = " << x_p1_fin << std::endl;
        CHECK(AlmostEqual(x_p[0], x_p1_fin, 0.01f));
        std::cout << "y_p1 = " << x_p[1] << ", y_p1 = " << y_p1_fin << std::endl;
        CHECK(AlmostEqual(x_p[1], y_p1_fin, 0.01f));

        sim.mblock()->metric.x_Code2Cart(
          {(real_t)(sim.mblock()->particles[1].i1_h(0)) + sim.mblock()->particles[1].dx1_h(0),
           (real_t)(sim.mblock()->particles[1].i2_h(0)) + sim.mblock()->particles[1].dx2_h(0)},
          x_p);
    ...
    //     CHECK(AlmostEqual(sim.mblock()->timestep(), 0.0026240291f));
    ...
    //         mblock->metric.v_Hat2Cntrv({i_ + HALF, j_ + HALF}, {ZERO, ZERO, bz_hat},
    //         bz_cntr);
    ...
    //     CHECK(AlmostEqual(
    //       sim.mblock()->em_h(100 + N_GHOSTS, 150 + N_GHOSTS, em::ex1),
    //       -191.516f));
    //     CHECK(AlmostEqual(
    //       sim.mblock()->em_h(10 + N_GHOSTS, 54 + N_GHOSTS, em::ex2), 114.057f));
    //     CHECK(AlmostEqual(
    //       sim.mblock()->em_h(60 + N_GHOSTS, 186 + N_GHOSTS, em::bx3), 246.654f));

    //     real_t all_zeros = 0.0f;
    //     for (std::size_t i {0}; i < 256; ++i) {
    //       for (std::size_t j {0}; j < 256; ++j) {
    //         all_zeros += math::abs(
    //           sim.mblock()->em_h(i + N_GHOSTS, j + N_GHOSTS, em::ex3));
    //         all_zeros += math::abs(
    //           sim.mblock()->em_h(i + N_GHOSTS, j + N_GHOSTS, em::bx1));
    //         all_zeros += math::abs(
    //           sim.mblock()->em_h(i + N_GHOSTS, j + N_GHOSTS, em::bx2));
    //       }
    //     }
    ...
    //     CHECK(AlmostEqual(sim.mblock()->timestep(), 0.010496116243f));
    //     CHECK(AlmostEqual(sim.sim_params()->sigma0(), 0.01f));
    ...
    //     CHECK(AlmostEqual(
    //       sim.mblock()->cur_h(161 + N_GHOSTS, 8 + N_GHOSTS, cur::jx2)
    //         + sim.mblock()->cur_h(162 + N_GHOSTS, 8 + N_GHOSTS, cur::jx2),
    //       -55.23417f));
    //     CHECK(AlmostEqual(
    //       sim.mblock()->cur_h(161 + N_GHOSTS, 8 + N_GHOSTS, cur::jx3)
    //         + sim.mblock()->cur_h(162 + N_GHOSTS, 8 + N_GHOSTS, cur::jx3)
    //         + sim.mblock()->cur_h(161 + N_GHOSTS, 9 + N_GHOSTS, cur::jx3)
    //         + sim.mblock()->cur_h(162 + N_GHOSTS, 9 + N_GHOSTS, cur::jx3),
    //       -31.889464f));

    //     CHECK(AlmostEqual(
    //       sim.mblock()->cur_h(15 + N_GHOSTS, 52 + N_GHOSTS, cur::jx1)
    //         + sim.mblock()->cur_h(15 + N_GHOSTS, 53 + N_GHOSTS, cur::jx1)
    //         + sim.mblock()->cur_h(16 + N_GHOSTS, 53 + N_GHOSTS, cur::jx1)
    //         + sim.mblock()->cur_h(16 + N_GHOSTS, 54 + N_GHOSTS, cur::jx1),
    //       -1.6f * 45.098512f));

    //     CHECK(AlmostEqual(
    //       sim.mblock()->cur_h(15 + N_GHOSTS, 52 + N_GHOSTS, cur::jx2)
    //         + sim.mblock()->cur_h(16 + N_GHOSTS, 52 + N_GHOSTS, cur::jx2)
    //         + sim.mblock()->cur_h(16 + N_GHOSTS, 53 + N_GHOSTS, cur::jx2)
    //         + sim.mblock()->cur_h(17 + N_GHOSTS, 53 + N_GHOSTS, cur::jx2),
    //       -1.6f * 45.098512f));
