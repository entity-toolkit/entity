...
struct RadialDist {
  ...
  RadialDist(const M& metric) : metric { metric } {}

  Inline auto operator()(const coord_t<M::Dim>& x_Code) const -> real_t {
    coord_t<M::Dim> x_Sph { ZERO };
    metric.template convert<Crd::Cd, Crd::Sph>(x_Code, x_Sph);
    auto r { ZERO };
...
auto main(int argc, char* argv[]) -> int {
  ...
  try {
    ...
    RadialDist<Minkowski<Dim::_2D>> r1 { m1 };

    Minkowski<Dim::_3D> m2 {
      {            10,            10,            30 },
      { { -1.0, 1.0 }, { -1.0, 1.0 }, { -3.0, 3.0 } }
    };
    RadialDist<Minkowski<Dim::_3D>> r2 { m2 };

    Kokkos::parallel_for(
      "RadialDist 2D",
      CreateRangePolicy<Dim::_2D>({ 0, 0 }, { 10, 10 }),
      Caller<RadialDist<Minkowski<Dim::_2D>>, Minkowski<Dim::_2D>>(r1, m1));

    Kokkos::parallel_for(
      "RadialDist 3D",
      CreateRangePolicy<Dim::_3D>({ 0, 0, 0 }, { 10, 10, 30 }),
      Caller<RadialDist<Minkowski<Dim::_3D>>, Minkowski<Dim::_3D>>(r2, m2));
