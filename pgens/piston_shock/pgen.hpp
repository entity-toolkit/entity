...
namespace user {
  ...
  struct PGen : public arch::ProblemGenerator<S, M> {
    ...
    inline void InitPrtls(Domain<S, M>& domain) {
      ...
      piston_position_cd = domain.mesh.metric.template convert<1, Crd::Ph, Crd::Cd>(
        piston_position);
      ...
      piston_velocity_cd = domain.mesh.metric.template transform<1, Idx::XYZ, Idx::U>(
        xp_Cd,
        piston_velocity);
      ...
      // inject particles
      arch::InjectUniformMaxwellians<S, M>(params, domain, ONE, temperatures, { 1, 2 });
    }
