...
namespace ntt {
  ...
  struct ThermalBackground : public EnergyDistribution<D, S> {
    ...
    Inline void operator()(const coord_t<D>& x_ph,
      ...
      if constexpr (D == Dim2) {
        coord<D> x_cd { ZERO };
        mblock.metric.x_Code2Phys(x_ph, x_cd);
        const auto i1  = static_cast<int>(x_cd[0]) + N_GHOSTS;
        const auto i2  = static_cast<int>(x_cd[1]) + N_GHOSTS;
        const auto bx1 = mblock.em(i1, i2, em::bx1);
        const auto bx2 = mblock.em(i1, i2, em::bx1);
        mblock.metric.v3_Cntr2Hat(x_cd, { bx1, bx2, ZERO }, b_int);
      }
      ...
      //
      // bnorm = math::sqrt(
      //   b_int[0] * b_int[0] + b_int[1] * b_int[1] + b_int[2] * b_int[2]);
  ...
  struct ProblemGenerator : public PGen<D, S> {
    ...
    Inline auto ext_force_x1(const real_t& time, const coord_t<PrtlCoordD>& x_ph) const
      -> real_t override {
      return -m_gravity * SQR(m_psr_Rstar / x_ph[0]) * 0.5 *
             (1.0 -
              math::tanh((x_ph[0] / m_psr_Rstar - (m_psr_Rstar + m_atm_h * 2.0)) /
                         (0.0125 * m_psr_Rstar)));
      // return 0.0;
      // * ((time > 100.0)
      //                                       ? ONE
  ...
  Inline void surfaceRotationField(const coord_t<D>& x_ph,
    ...
    // _omega *= -0.5*math::tanh(50.*(-0.7853981633974483 + x_ph[1]))*(-1. + math::tanh(50.*(-0.9599310885968813
    // + x_ph[1]))*math::tanh(50.*(-0.6108652381980153 + x_ph[1]))); _omega *=
    // -0.5*math::tanh(25.*(-0.6108652381980153 + x_ph[1]))*(-1. + math::tanh(25.*(-0.7853981633974483
    // + x_ph[1]))*math::tanh(25.*(-0.4363323129985824 + x_ph[1]))); e_out[0]  = _omega * b_out[1] * x_ph[0] * math::sin(x_ph[1]);
    ...
    if (x_ph[1] < omega_centre) {
      _omega *= 1.0 /
                (1.0 +
                 math::exp(50.0 * (math::abs(x_ph[1] - omega_centre) - delta))) *
                (1.5 * (-omega_centre + x_ph[1])) / delta;
    } else {
      _omega *= 1.0 /
                (1.0 +
                 math::exp(50.0 * (math::abs(x_ph[1] - omega_centre) - delta))) *
    ...
    // _omega *= (1.0 / (1.0 + math::exp(50.0 * (math::abs(x_ph[1] -
    // omega_centre) - delta))) * (1.5*(-omega_centre + x_ph[1]))/delta) + 1.0 /
    // (1.0 + math::exp(50.0 * (math::abs(x_ph[1] - omega_centreB) - delta))) *
    // (1.5*(-omega_centreB + x_ph[1]))/delta;

    // if (x_ph[1] < 0.5 * pival) {
    // auto sigma  = (x_ph[1] - 0.5 * pival) / (0.25 * pival);
    // _omega     *= sigma * math::exp((1.0 - SQR(SQR(sigma))) / 4.0);
    // _omega *= sigma * math::exp((1.0 - SQR(SQR(sigma))) / 4.0) * (0.5 + 0.5*math::tanh(58.90486225480862
    // - 50.*x_ph[1])*math::tanh(98.17477042468103 - 50.*x_ph[1]));
  ...
  inline void ProblemGenerator<Dim2, PICEngine>::UserInitParticles(
    ...
    // inject particles in the atmosphere
    {
      ...
      //     const auto i1_ = static_cast<int>(i1) - N_GHOSTS;
      //     const auto i2_ = static_cast<int>(i2) - N_GHOSTS;
      //     const vec_t<Dim3> xi { i1_, i2_ , 0};
      //     coord_t<Dim3>     x_ph { ZERO };
      //     mblock.metric.x_Code2Cart(xi, x_ph);

      //     m_ppc_per_spec(i1_, i2_) = math::exp(40.*(-SQR(-1.25 + x_ph[0]) - SQR(-1.25 + x_ph[2])));
  ...
  inline void ProblemGenerator<Dim2, PICEngine>::UserDriveParticles(
    ...
    //         //     qres = SQR(eph_LF) / omegares * math::exp(-0.5 * xres *
    //         xres / ((gfac) * (gfac))) /
    //         //            sqrt(2.0 * M_PI * ((gfac) * (gfac))) * 1.0 /
    //         (math::exp(eph_LF/tpeak) - 1.0);
    //         //   }
    ...
    //           + bz0_rest * bz0_rest) / bq; auto eph_LF = bbq / (gamma * (1.0
