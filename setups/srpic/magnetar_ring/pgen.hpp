...
namespace user {
  ...
  struct Blob : public arch::EnergyDistribution<S, M> {
    Blob(const M&                   metric,
         const real_t& gamma)
    ...
    Inline void operator()(const coord_t<M::Dim>& x_Ph,
      ...
      auto v1 = x_Cart[0] / x_Ph[0];
      auto v2 = x_Cart[1] / x_Ph[0];
      auto v3 = x_Cart[2] / x_Ph[0];
  ...
  template <SimEngine::type S, class M>
  struct BlobDistribution : public arch::SpatialDistribution<S, M> {
    BlobDistribution(const M&                   metric,
                      const std::vector<real_t>& x_cntr,
                      real_t                     sigma)
      : arch::SpatialDistribution<S, M> { metric } {
        sig = sigma;
        std::copy(x_cntr.begin(), x_cntr.end(), x_c);
      }
    ...
    Kokkos::parallel_for(
        "ScatterPhotons", species.rangeActiveParticles(), Lambda(index_t p) {
          if (tag(p) == ParticleTag::dead) {
            ...
            auto ndot = 1.41631 * math::pow(10.0, 18) * SQR(Rstar_/xPh[0]) * sqrt(beta_sq) / gamma
