...
namespace user {
  ...
  private:
    const real_t time, Omega, OmegaLT, comp;
  };
  ...
  struct PGen : public arch::ProblemGenerator<S, M> {
    ...
    void CustomPartEvolution(std::size_t step, long double time, Domain<S, M>& domain) {
      ...
      const auto ph_thres1 {params.template get<real_t>("setup.ph_thres1") };
      const auto ph_thres2 {params.template get<real_t>("setup.ph_thres2") };
