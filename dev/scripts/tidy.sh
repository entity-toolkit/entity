...
# 	while read f; do
# 		clang-tidy -p "$build_dir" "$f" || exit 1
# 	done
...
# out_dir="tidy"
# project_root="$(pwd)"
# allowed_re="${3:-^${project_root}/(src|pgens)/}"
...
# Build file list: all files from compile_commands.json, optionally filtered
