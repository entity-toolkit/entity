...
//   }
  ...
  Inline bool SigmaCrit<Dim2, GRPICEngine>::operator()(
    ...
    vec_t<Dim3> b_cov { (this->m_mblock).em(i1, i2, em::bx1), (this->m_mblock).em(i1, i2, em::bx2), (this->m_mblock).em(i1, i2, em::bx3) };
    vec_t<Dim3> b_cntrv { ZERO };
    (this->m_mblock).metric.v3_Cov2Cntrv(xi, b_cov, b_cntrv);
    real_t sigma { DOT(b_cov[0], b_cov[1], b_cov[2], b_cntrv[0], b_cntrv[1], b_cntrv[2]) / ((this->m_mblock).buff(i1, i2, 0)) };

    // return (sigma > _sigma_min) && (density < _density_min * nGJ);
...
//   WaitAndSynchronize();
  ...
  InjectNonUniform<Dim2, GRPICEngine, ColdDist, SigmaCrit>( 
    ...
    { 1, 2 }, 
    m_ppc_per_spec, 
    { inj_rmin, inj_rmax, 0.0, (ntt::constant::PI)});
