...
\frac{\partial B^i}{c\partial t} &= -\frac{1}{\sqrt{h}} \left[\partial_j (B_k) - \partial_k (E_j)\right],
\end{aligned}
$$
...
{{< hint info >}}
**Conversion to orthonormal basis (flat orthogonal metric)**\
Notice that in general the basis where this metric and field components are defined is not orthonormal. In general, one might need to convert the contravariant vector components, $A^i$, to orthonormal basis: $A^\hat{i}$. In case of flat orthogonal metric this can be written as (no summation implied):
$$
A^\hat{i} = \sqrt{h_{i}} A^i
$$
