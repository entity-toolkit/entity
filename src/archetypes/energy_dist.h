 ...
 *   - arch::PowerlawDist<> : arch::EnergyDistribution<>
...
namespace arch {
  ...
  template <SimEngine::type S, class M>
  struct PowerlawDist : public EnergyDistribution<S, M> {
    ...
    Inline void operator()(const coord_t<M::Dim>& x_Code,
         ...
         // Power-law distribution from uniform distribution (see https://mathworld.wolfram.com/RandomNumber.html)
         if (pl_ind != -1.0) {
