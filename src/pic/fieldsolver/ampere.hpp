...
Inline void Ampere<TWO_D>::operator()(const index_t i, const index_t j) const {
  m_mblock.ex1(i, j)
      = m_mblock.ex1(i, j) + coeff_x2 * (-m_mblock.bx3(i, j - 1) + m_mblock.bx3(i, j));
  m_mblock.ex2(i, j)
      = m_mblock.ex2(i, j) + coeff_x1 * (m_mblock.bx3(i - 1, j) - m_mblock.bx3(i, j));
  m_mblock.ex3(i, j) = m_mblock.ex3(i, j) + coeff_x2 * (m_mblock.bx1(i, j - 1) - m_mblock.bx1(i, j))
