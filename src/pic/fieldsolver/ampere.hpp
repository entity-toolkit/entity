...
public:
  Ampere(const Meshblock<D>& m_mblock_, const real_t& coeff_x1_, const real_t& coeff_x2_, const real_t& coeff_x3_)
...
Inline void Ampere<ONE_D>::operator()(const index_t i) const {
  m_mblock.ex2(i) = m_mblock.ex2(i) + coeff_x1 * (m_mblock.bx3(i - 1) - m_mblock.bx3(i));
  m_mblock.ex3(i) = m_mblock.ex3(i) + coeff_x1 * (-m_mblock.bx2(i - 1) + m_mblock.bx2(i));
}

template <>
Inline void Ampere<TWO_D>::operator()(const index_t i, const index_t j) const {
  m_mblock.ex1(i, j) = m_mblock.ex1(i, j) + coeff_x2 * (-m_mblock.bx3(i, j - 1) + m_mblock.bx3(i, j));
  m_mblock.ex2(i, j) = m_mblock.ex2(i, j) + coeff_x1 * (m_mblock.bx3(i - 1, j) - m_mblock.bx3(i, j));
  m_mblock.ex3(i, j) = m_mblock.ex3(i, j)
                       + coeff_x2 * (m_mblock.bx1(i, j - 1) - m_mblock.bx1(i, j))
                       + coeff_x1 * (-m_mblock.bx2(i - 1, j) + m_mblock.bx2(i, j));
}
...
Inline void Ampere<THREE_D>::operator()(const index_t i, const index_t j, const index_t k) const {
  m_mblock.ex1(i, j, k) = m_mblock.ex1(i, j, k)
                          + coeff_x3 * (m_mblock.bx2(i, j, k - 1) - m_mblock.bx2(i, j, k))
                          + coeff_x2 * (-m_mblock.bx3(i, j - 1, k) + m_mblock.bx3(i, j, k));
  m_mblock.ex2(i, j, k) = m_mblock.ex2(i, j, k)
                          + coeff_x1 * (m_mblock.bx3(i - 1, j, k) - m_mblock.bx3(i, j, k))
                          + coeff_x3 * (-m_mblock.bx1(i, j, k - 1) + m_mblock.bx1(i, j, k));
  m_mblock.ex3(i, j, k) = m_mblock.ex3(i, j, k)
                          + coeff_x2 * (m_mblock.bx1(i, j - 1, k) - m_mblock.bx1(i, j, k))
                          + coeff_x1 * (-m_mblock.bx2(i - 1, j, k) + m_mblock.bx2(i, j, k));
}
