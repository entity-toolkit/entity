...
namespace ntt {
  ...
  Inline void FaradayCurvilinear<ONE_D>::operator()(const index_t i) const {
    ...
    real_t inv_sqrt_detH_iP {
      ONE / m_mblock.m_coord_system->sqrt_det_h(x1 + 0.5 * dx1)
    };
    real_t h2_i {
      m_mblock.m_coord_system->hx2(x1)
    };
    real_t h2_iP1 {
      m_mblock.m_coord_system->hx2(x1 + dx1)
    };
    real_t h3_i {
      m_mblock.m_coord_system->hx3(x1)
    };
    real_t h3_iP1 {
      m_mblock.m_coord_system->hx3(x1 + dx1)
    };

    m_mblock.em_fields(i, fld::bx2) += inv_sqrt_detH_iP * coeff_x1 * (
                                          h3_iP1 * m_mblock.em_fields(i + 1, fld::ex3) - h3_i * m_mblock.em_fields(i, fld::ex3)
                                        );
    m_mblock.em_fields(i, fld::bx3) += inv_sqrt_detH_iP * coeff_x1 * (
                                          h2_i * m_mblock.em_fields(i, fld::ex2) - h2_iP1 * m_mblock.em_fields(i + 1, fld::ex2)
                                        );
  }
  ...
  Inline void FaradayCurvilinear<TWO_D>::operator()(const index_t i, const index_t j) const {
    ...
    real_t inv_sqrt_detH_iPj {
      ONE / m_mblock.m_coord_system->sqrt_det_h(x1 + 0.5 * dx2, x2)
    };
    real_t inv_sqrt_detH_ijP {
      ONE / m_mblock.m_coord_system->sqrt_det_h(x1, x2 + 0.5 * dx2)
    };
    real_t inv_sqrt_detH_iPjP {
      ONE / m_mblock.m_coord_system->sqrt_det_h(x1 + 0.5 * dx1, x2 + 0.5 * dx2)
    };
    real_t h1_iPjP1 {
      m_mblock.m_coord_system->hx1(x1 + 0.5 * dx1, x2 + dx2)
    };
    real_t h1_iPj {
      m_mblock.m_coord_system->hx1(x1 + 0.5 * dx1, x2)
    };
    real_t h2_iP1jP {
      m_mblock.m_coord_system->hx2(x1 + dx1, x2 + 0.5 * dx2)
    };
    real_t h2_ijP {
      m_mblock.m_coord_system->hx2(x1, x2 + 0.5 * dx2)
    };
    real_t h3_ij {
      m_mblock.m_coord_system->hx3(x1, x2)
    };
    real_t h3_iP1j {
      m_mblock.m_coord_system->hx3(x1 + dx1, x2)
    };
    real_t h3_ijP1 {
      m_mblock.m_coord_system->hx3(x1, x2 + dx2)
    };

    m_mblock.em_fields(i, j, fld::bx1) += inv_sqrt_detH_ijP * coeff_x2 * (
                                              h3_ij * m_mblock.em_fields(i, j, fld::ex3) - h3_ijP1 * m_mblock.em_fields(i, j + 1, fld::ex3)
                                            );
    m_mblock.em_fields(i, j, fld::bx2) += inv_sqrt_detH_iPj * coeff_x1 * (
                                              h3_iP1j * m_mblock.em_fields(i + 1, j, fld::ex3) - h3_ij * m_mblock.em_fields(i, j, fld::ex3)
                                            );
    m_mblock.em_fields(i, j, fld::bx3) += inv_sqrt_detH_iPjP * (coeff_x2 * (
