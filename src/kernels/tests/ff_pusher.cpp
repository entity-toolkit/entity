...
#define DERIVATIVE(func, x)                                                    \
  ((func({ x + epsilon }) - func({ x - epsilon })) / (TWO * epsilon))
...
void testFFPusher(const std::vector<std::size_t>&      res,
  ...
  const real_t d_eta_inv { nx1 / (metric.r2eta(extent[0].second) -
                                  metric.r2eta(extent[0].first)) };
  ...
  auto efield = ndfield_t<Dim::_1D, 1> { "efield", res[0] + 2 * N_GHOSTS };
  ...
  Kokkos::parallel_for(
    "init efield",
    range_ext,
    Lambda(index_t i1) { efield(i1, em::ex1) = ZERO; });
  ...
  Kokkos::parallel_for(
    ...
    // clang-format off
    kernel::gr::Pusher_kernel<BoyerLindqTP<Dim::_1D>>(efield,
                                                      i1, i1_prev, dx1, dx1_prev, px1,
                                                      tag, metric, -coeff, dt, nx1,
                                                      1e-2, 30, boundaries));
  // clang-format on
  ...
  Kokkos::parallel_for(
    "init efield",
    range_ext,
    Lambda(index_t i1) { efield(i1, em::ex1) = 99.0; });
  ...
  real_t pp_e_p_exp[10] { 1.12420507263443, 1.18040523823437, 1.27055279565053,
                          ...
                          170.710945349036 };
  real_t pp_e_n_exp[10] { 0.886850044532529, 0.837694736928196,
                          0.754741504814654, 0.607238475100313,
                          0.325753270778111, -0.266166995807237,
                          -1.67869862572720, -5.82318047383146,
                          -23.1877218243589, -166.709733472388 };
