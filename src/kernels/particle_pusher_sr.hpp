  ...
  public:
    ...
    Inline void synchrotronDrag(index_t&               p,
      ...
      ux3(p) += coeff_sync * (kappaR[2] - gamma_prime_sqr * u_prime[2] * chiR_sqr);
        ...
        // CG:  Half-step Hubble drag / anti-drag for the Box metric 
    Inline void hubble_half_step(index_t& p) const {
      // Only active for the expanding/compressing Box metric in Cartesian coords
      if constexpr (M::MetricType == ntt::Metric::Box &&
                    M::CoordType  == Coord::Cart) {

        const real_t half_dt = HALF * dt;

        const real_t Hx = metric.get_Hx();
        const real_t Hy = metric.get_Hy();
        const real_t Hz = metric.get_Hz();
	        // DEBUG (only print for 1 particle to avoid spam)
        if (p == 0) {
            Kokkos::printf(
               "pusher Hubble: Hx=%e Hy=%e Hz=%e half_dt=%e\n",
               (double)Hx, (double)Hy, (double)Hz, (double)half_dt
            );
            Kokkos::printf(
               "pusher Before: ux=%e uy=%e uz=%e\n",
               (double)ux1(p), (double)ux2(p), (double)ux3(p)
            );
        }
        ...
        // x–component
        if (Hx != ZERO) {
          const real_t factor_x = math::exp(-Hx * half_dt);
          ux1(p) *= factor_x;
        }

        // y–component (only if 2D or 3D)
        if constexpr (D == Dim::_2D || D == Dim::_3D) {
          if (Hy != ZERO) {
            const real_t factor_y = math::exp(-Hy * half_dt);
            ux2(p) *= factor_y;
          }
        }

        // z–component (only if 3D)
        if constexpr (D == Dim::_3D) {
          if (Hz != ZERO) {
            const real_t factor_z = math::exp(-Hz * half_dt);
            ux3(p) *= factor_z;
          }
	}
	        // DEBUG after applying the drag
        if (p == 0) {
            Kokkos::printf(
               "DEBUG pusher After: ux=%e uy=%e uz=%e\n",
               (double)ux1(p), (double)ux2(p), (double)ux3(p)
            );
        }
      }
    ...
    Inline void operator()(index_t p) const {
      ...
      if (p < 4) {
        ...
        Kokkos::printf(
          "[pusher_sr] step≈%d p=%d xp=(%e,%e,%e)\n",
          (int)step_est, (int)p,
          (double)xp_Cd[0],
          (M::PrtlDim >= Dim::_2D ? (double)xp_Cd[1] : 0.0),
          (M::PrtlDim == Dim::_3D ? (double)xp_Cd[2] : 0.0));

        Kokkos::printf(
          "[pusher_sr]   E'(grid)=(%e,%e,%e)  B'(grid)=(%e,%e,%e)\n",
          (double)ei[0], (double)ei[1], (double)ei[2],
          (double)bi[0], (double)bi[1], (double)bi[2]);

        Kokkos::printf(
          "[pusher_sr]   E_phys=(%e,%e,%e)    B_phys=(%e,%e,%e)\n",
          (double)ei_Cart[0], (double)ei_Cart[1], (double)ei_Cart[2],
          (double)bi_Cart[0], (double)bi_Cart[1], (double)bi_Cart[2]);
      } 
      ...
      // CGFirst half-step Hubble drag / anti-drag
      hubble_half_step(p);
      ...
      if (GCA) {
       ...
       // CG Second half-step Hubble drag / anti-drag
      hubble_half_step(p);
      // cooling
