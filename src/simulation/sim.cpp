...
namespace ntt {
  Simulation::Simulation(Dimension dim, CoordinateSystem coord_sys) : m_dimension(dim), m_coord_system(coord_sys) {
    // check compatibility
    try {
      if (((m_dimension == ONE_D) && (m_coord_system != CARTESIAN)) ||
          ((m_dimension == TWO_D) && ((m_coord_system == SPHERICAL) || (m_coord_system == LOG_SPHERICAL))) ||
  ...
  void Simulation::parseInput(int argc, char *argv[]) {
    ...
    m_input_params.set_input_filename(cl_args.getArgument("-input", DEF_input_filename));
