...
namespace ntt {
  namespace arrays {
    ...
    void OneDArray<T>::allocate(std::size_t n1_) {
      ...
      this->n1 = n1_; this->n1_full = n1_ + 2 * N_GHOSTS;
    ...
    void TwoDArray<T>::allocate(std::size_t n1_, std::size_t n2_) {
      ...
      this->n1 = n1_; this->n1_full = n1_ + 2 * N_GHOSTS;
      this->n2 = n2_; this->n2_full = n2_ + 2 * N_GHOSTS;
    ...
    void ThreeDArray<T>::allocate(std::size_t n1_, std::size_t n2_, std::size_t n3_) {
      ...
      this->n1 = n1_; this->n1_full = n1_ + 2 * N_GHOSTS;
      this->n2 = n2_; this->n2_full = n2_ + 2 * N_GHOSTS;
      this->n3 = n3_; this->n3_full = n3_ + 2 * N_GHOSTS;
    ...
    // filler
    template<class T>
    void OneDArray<T>::fillWith(T value, bool fill_ghosts) {
    ...
    template<class T>
    void TwoDArray<T>::fillWith(T value, bool fill_ghosts) {
    ...
    template<class T>
    void ThreeDArray<T>::fillWith(T value, bool fill_ghosts) {
    ...
    // size getter
    template<class T>
    std::size_t OneDArray<T>::get_size(short int n) {
      assert((n == 1) && "# Error: there is only 1 dimension for 1d array.");
      return this->n1;
    }
    template<class T>
    std::size_t TwoDArray<T>::get_size(short int n) {
    ...
    template<class T>
    std::size_t ThreeDArray<T>::get_size(short int n) {
    ...
    // memory usage
    template<class T>
    std::size_t OneDArray<T>::getSizeInBytes() {
      if (this->m_allocated) {
    ...
    template<class T>
    std::size_t TwoDArray<T>::getSizeInBytes() {
      if (this->m_allocated) {
    ...
    template<class T>
    std::size_t ThreeDArray<T>::getSizeInBytes() {
      if (this->m_allocated) {
