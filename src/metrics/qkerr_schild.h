  ...
  public:
    ...
    Inline auto g_(const coord_t<D>& x) const -> real_t {
      ...
      if constexpr (i == 0 && j == 0) {
        // g_00
        const real_t theta { eta2theta(x[1] * deta + eta_min) };
        return - (ONE - z(r0 + math::exp(x[0] * dchi + chi_min), theta)); 
      } else if constexpr ((i == 0 && j == 1) ||  (i == 1 && j == 0)) {
        // g_01 or g_10
        const real_t theta { eta2theta(x[1] * deta + eta_min) };
        return dchi * math::exp(x[0] * dchi + chi_min) * z(r0 + math::exp(x[0] * dchi + chi_min), theta);
        ...
        // g_11
        const real_t theta { eta2theta(x[1] * deta + eta_min) };
        return SQR(dchi) * math::exp(TWO * (x[0] * dchi + chi_min)) * (1 + z(r0 + math::exp(x[0] * dchi + chi_min), theta));
      } else if constexpr (i == 2 && j == 2) {
        // g_22
        return h_<2, 2>(x);
      } else if constexpr (i == 3 && j == 3) {
    ...
    Inline auto g(const coord_t<D>& x) const -> real_t {
      ...
      if constexpr (i == 0 && j == 0) {
        ...
        // g^01 or g^10
        const real_t theta { eta2theta(x[1] * deta + eta_min) };
        return (math::exp(-(x[0] * dchi + chi_min)) * dchi_inv) * z(r0 + math::exp(x[0] * dchi + chi_min), theta);
      } else if constexpr (i == 1 && j == 1) {
        // g^11
        const real_t theta { eta2theta(x[1] * deta + eta_min) };
        return (math::exp(-TWO * (x[0] * dchi + chi_min)) / SQR(dchi)) * Delta(r0 + math::exp(x[0] * dchi + chi_min))/ Sigma(r0 + math::exp(x[0] * dchi + chi_min), theta);
      } else if constexpr (i == 2 && j == 2) {
        // g^22
        return h<2, 2>(x);
      } else if constexpr (i == 3 && j == 3) {
