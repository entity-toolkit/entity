...
namespace ntt {
  ...
  if ((j<=6) && (i<=6)) {
  printf("%f %f %f %lu %lu\n",m_mblock.em0(i, j, em::bx3), 
  m_mblock.metric.sqrt_det_h({i_ + HALF, j_ + HALF}), m_mblock.metric.polar_area({i_ + HALF, j_ + HALF}), i, j);
  //m_mblock.aux(i, j + 1, em::ex1), m_mblock.aux(i, j, em::ex1), i, j);
  }
