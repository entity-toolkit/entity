...
namespace out {
  ...
  auto InterpretComponents(const std::vector<std::string>& comps)
    ...
    std::vector<std::vector<uint8_t>> comps_ints;
    if (comps_int.size() == 1) {
      auto c = comps_int[0];
      if (c < 0) {
        for (int i { 0 }; i < 3; ++i) {
          comps_ints.push_back({ static_cast<uint8_t>(i + 1) });
        }
      } else {
        raise::ErrorIf((c > 3) || (c == 0), "Invalid field name", HERE);
        comps_ints.push_back({ static_cast<uint8_t>(c) });
      }
    } else {
      auto c1 = comps_int[0];
      auto c2 = comps_int[1];
      if (c1 < 0) {
        for (int i { 0 }; i < 3; ++i) {
          if (c2 < 0) {
            if (c2 == c1) {
              comps_ints.push_back(
                { static_cast<uint8_t>(i + 1), static_cast<uint8_t>(i + 1) });
            } else {
              for (int j { i }; j < 3; ++j) {
                comps_ints.push_back(
                  { static_cast<uint8_t>(i + 1), static_cast<uint8_t>(j + 1) });
              }
            }
          } else {
            comps_ints.push_back(
              { static_cast<uint8_t>(i + 1), static_cast<uint8_t>(c2) });
          }
        }
      } else {
        if (c2 < 0) {
          for (int j { 0 }; j < 3; ++j) {
            comps_ints.push_back(
              { static_cast<uint8_t>(c1), static_cast<uint8_t>(j + 1) });
          }
        } else {
          comps_ints.push_back(
