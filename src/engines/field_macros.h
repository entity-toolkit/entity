...
#define ATOMIC_JX1(...)                                                        \
  GET_MACRO(__VA_ARGS__, ATOMIC_JX1_3D, ATOMIC_JX1_2D, ATOMIC_JX1_1D, )        \
  (__VA_ARGS__)
#define ATOMIC_JX1_1D(I) (cur_access((I) + N_GHOSTS, cur::jx1))
#define ATOMIC_JX1_2D(I, J)                                                    \
  (cur_access((I) + N_GHOSTS, (J) + N_GHOSTS, cur::jx1))
#define ATOMIC_JX1_3D(I, J, K)                                                 \
  (cur_access((I) + N_GHOSTS, (J) + N_GHOSTS, (K) + N_GHOSTS, cur::jx1))

#define ATOMIC_JX2(...)                                                        \
  GET_MACRO(__VA_ARGS__, ATOMIC_JX2_3D, ATOMIC_JX2_2D, ATOMIC_JX2_1D, )        \
  (__VA_ARGS__)
#define ATOMIC_JX2_1D(I) (cur_access((I) + N_GHOSTS, cur::jx2))
#define ATOMIC_JX2_2D(I, J)                                                    \
  (cur_access((I) + N_GHOSTS, (J) + N_GHOSTS, cur::jx2))
#define ATOMIC_JX2_3D(I, J, K)                                                 \
  (cur_access((I) + N_GHOSTS, (J) + N_GHOSTS, (K) + N_GHOSTS, cur::jx2))

#define ATOMIC_JX3(...)                                                        \
  GET_MACRO(__VA_ARGS__, ATOMIC_JX3_3D, ATOMIC_JX3_2D, ATOMIC_JX3_1D, )        \
  (__VA_ARGS__)
#define ATOMIC_JX3_1D(I) (cur_access((I) + N_GHOSTS, cur::jx3))
#define ATOMIC_JX3_2D(I, J)                                                    \
  (cur_access((I) + N_GHOSTS, (J) + N_GHOSTS, cur::jx3))
#define ATOMIC_JX3_3D(I, J, K)                                                 \
  ...
  #define set_em_E_1d(MBLOCK, I, XCODE, COMP, COMPI, FUNC, ...)                \
    {                                                                          \
      ...
      (MBLOCK).metric.x_Code2Cart((XCODE), x_ph);                              \
  ...
  #define set_em_E_2d(MBLOCK, I, J, XCODE, COMP, COMPI, FUNC, ...)             \
    {                                                                          \
      ...
      vec_t<Dim3>   e_cntrv { ZERO };                                          \
      coord_t<Dim2> x_ph { ZERO };                                             \
      (MBLOCK).metric.x_Code2Sph((XCODE), x_ph);                               \
      FUNC(x_ph, e_hat, b_hat, __VA_ARGS__);                                   \
      (MBLOCK).metric.v3_Hat2Cntrv((XCODE), e_hat, e_cntrv);                   \
      (MBLOCK).em((I), (J), (COMP)) = e_cntrv[(COMPI)];                        \
    }
