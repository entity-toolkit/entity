...
namespace ntt {
  ...
  struct ProblemGenerator : public PGen<D, S> {
    ...
    Inline auto ext_force_x1(const real_t&, const coord_t<D>& x_ph) const -> real_t override {
      return -m_g0 * SQR(m_Rstar / x_ph[0]) * (x_ph[0] < m_Rstar + m_h * 2);
    }
  ...
  inline void ProblemGenerator<Dim2, PICEngine>::UserDriveFields(
    ...
    const auto _omega  = (time < spinup_time) ? (time / spinup_time) * spin_omega : spin_omega;
  ...
  /*  template <>
  inline void ProblemGenerator<Dim2, PICEngine>::UserDriveCurrents(
    const real_t& time, const SimulationParams&, Meshblock<Dim2, PICEngine>& mblock) {
    // ... drive currents ...
    // mblock.cur(i, j, k, cur::jx1) = 0.0;
    }*/
  ...
  inline void ProblemGenerator<Dim2, PICEngine>::UserDriveParticles(
    ...
    /*{
      ...
      const auto ph_thres { params.get<real_t>("problem", "ph_threshold") };
      const auto rad_radius { params.get<real_t>("problem", "rad_radius") };
      const auto RLC { ONE / spin_omega };
      const auto curv_coeff { -(TWO * mblock.timestep() / params.larmor0()) * b_surf
                              * math::pow(m_Rstar / RLC, (real_t)(1.5)) / SQR(SQR(gamma_curv)) };
      const auto curv_ph_coeff {ONE / CUBE(gamma_curv_ph) };
      auto&      photons     = mblock.particles[2];
      auto       random_pool = *(mblock.random_pool_ptr);
      for (std::size_t s { 0 }; s < 2; ++s) {
        array_t<std::size_t> ph_ind("ph_ind");
        auto&                species   = mblock.particles[s];
        const auto           ph_offset = photons.npart();
        Kokkos::parallel_for(
          "CurvRadiationEmission", species.rangeActiveParticles(), Lambda(index_t p) {
            if (species.tag(p) != ParticleTag::alive) {
              return;
            }
            const auto gamma_sqr { ONE + SQR(species.ux1(p)) + SQR(species.ux2(p))
                                   + SQR(species.ux3(p)) };
            if (gamma_sqr > (real_t)(3.0)) {
              const vec_t<Dim2> xi {species.i1(p) + species.dx1(p),
                                    species.i2(p) + species.dx2(p)};
              coord_t<Dim2>  x_ph { ZERO };
              mblock.metric.x_Code2Sph(xi, x_ph);
              const auto curv {1.0}; // rho(x)/rho_0
              typename RandomNumberPool_t::generator_type rand_gen = random_pool.get_state();
              const auto        inv_gamma { ONE / math::sqrt(gamma_sqr) };
              const vec_t<Dim3> beta { species.ux1(p) * inv_gamma,
                                       species.ux2(p) * inv_gamma,
                                       species.ux3(p) * inv_gamma };
              const auto beta_mag { math::sqrt(SQR(beta[0]) + SQR(beta[1]) + SQR(beta[2])) };
              const auto force { curv_coeff * CUBE(beta_mag) * SQR(gamma_sqr) / SQR(curv) };
              species.ux1(p) += force * beta[0];
              species.ux2(p) += force * beta[1];
              species.ux3(p) += force * beta[2];
              const auto ph_energy { curv_ph_coeff * gamma_sqr * math::sqrt(gamma_sqr) / curv };
              const auto ph_prob { math::abs(force) / ph_energy };

              if ((Random<real_t>(rand_gen) < ph_prob) && (ph_energy > ph_thres)) {// && 
		  //                (x_ph[0]<rad_radius)) {
                auto ph_p = Kokkos::atomic_fetch_add(&ph_ind(), 1);
                init_prtl_2d_i_di(photons,
                                  ph_offset + ph_p,
                                  species.i1(p),
                                  species.i2(p),
                                  species.dx1(p),
                                  species.dx2(p),
                                  ph_energy * beta[0] / beta_mag,
                                  ph_energy * beta[1] / beta_mag,
                                  ph_energy * beta[2] / beta_mag,
                                  species.weight(p));
              }
              random_pool.free_state(rand_gen);
            }
          });
        auto ph_ind_h = Kokkos::create_mirror(ph_ind);
        Kokkos::deep_copy(ph_ind_h, ph_ind);
        photons.setNpart(photons.npart() + ph_ind_h());
      }
    }*/
  }
}    // namespace ntt
