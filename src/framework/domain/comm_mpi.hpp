...
/**
 * attempt to receive directly into fields
 */
// template <typename Array, std::size_t... I>
// auto to_tuple_impl(const Array& a, std::index_sequence<I...>) {
//   return std::make_tuple(a[I]...);
// }

// template <typename T, std::size_t N, typename Indices =
// std::make_index_sequence<N>> auto to_tuple(const std::array<T, N>& a) {
//   return to_tuple_impl(a, Indices {});
// }
...
//   std::vector<std::pair<std::size_t, std::size_t>> send_init_list;
//   std::vector<std::pair<std::size_t, std::size_t>> recv_init_list;

//   if (send_rank >= 0) {
//     for (unsigned short d = 0; d < D; ++d) {
//       send_init_list.push_back({ send_slice[d].first, send_slice[d].second });
//       recv_init_list.push_back({ recv_slice[d].first, recv_slice[d].second });
//     }
//     send_init_list.push_back({ comps.first, comps.second });
//     recv_init_list.push_back({ comps.first, comps.second });
//   }

//   std::array<std::pair<std::size_t, std::size_t>, D + 1> send_array;
//   std::copy(send_init_list.begin(), send_init_list.end(), send_array.begin());

//   std::array<std::pair<std::size_t, std::size_t>, D + 1> recv_array;
//   std::copy(recv_init_list.begin(), recv_init_list.end(), recv_array.begin());

//   if (send_rank >= 0 && recv_rank >= 0) {
//     auto send_subview = std::apply(
//       [&](auto... args) {
//         return Kokkos::subview(fld, args...);
//       },
//       to_tuple(send_array));
//     auto recv_subview = std::apply(
//       [&](auto... args) {
//         return Kokkos::subview(fld, args...);
//       },
//       to_tuple(recv_array));

//     MPI_Sendrecv(send_subview.data(),
//                  nsend,
//                  mpi::get_type<real_t>(),
//                  send_rank,
//                  0,
