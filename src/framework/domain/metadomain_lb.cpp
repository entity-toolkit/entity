...
namespace ntt {
  ...
  bool negotiate_boundary_single(const std::vector<real_t>& N,
    ...
    real_t w1 { 0 };
    for (int k = left_start; k < mid; ++k) {
      w1 += N[k];
    }
    real_t w2 { 0 };
    for (int k = mid; k < right_end; ++k) {
      w2 += N[k];
    }

    // if the load difference is already within tolerance, do not move the boundary
    if (std::abs(w1 - w2) <= tol) {
      return false;
    }

    // minimum domain size
    const auto L_min = 2 * N_GHOSTS + 1;

    // try shifting the boundary in either direction and see if it improves the load balance, stop as soon as we see no improvement
    real_t best_diff { std::abs(w1 - w2) };
    int    best_shift { 0 };

    // more particles on the left -> try shifting boundary to the right (move some load from left to right)
    if (w1 > w2) {
      int    max_shift        = (mid - left_start) - L_min;
      real_t current_transfer = ZERO;
      for (int s = 1; s <= max_shift; ++s) {
        current_transfer += N[mid - s];
        real_t new_diff   = std::abs(
          (w1 - current_transfer) - (w2 + current_transfer));
      ...
      // more particles on the right -> try shifting boundary to the left (move some load from right to left)
    } else if (w2 > w1) {
  ...
  void Metadomain<S, M>::BalanceLoad(const SimulationParams& params) {
    ...
    // dimensions to load balance
    const auto lb_dims = params.template get<std::vector<int>>(
      "simulation.domain.load_balancing.dimensions");
    // maximum number of iterations for load balancing negotiation
    const auto lb_max_iters = static_cast<int>(params.template get<unsigned int>(
      "simulation.domain.load_balancing.max_iterations"));
    // tolerance for load balancing
    const auto lb_tol = static_cast<real_t>(params.template get<real_t>(
      "simulation.domain.load_balancing.tolerance"));
    ...
    // storage for updated boundaries, offsets, and number of cells for each dimension to be load balanced
    ...
    for (int dim : lb_dims) {
      ...
      for (int i = 0; i < nx_domains - 1; ++i) {
        ...
        bounds[i + 1]            = dom.offset_ncells()[d] +
                                   dom.mesh.n_active(static_cast<int>(d));
      }
      ...
      for (int iter = 1; iter <= lb_max_iters; ++iter) {
        ...
        for (int i = 1; i < nx_domains; i += 2) {
          moved_global |= negotiate_boundary_single(N, bounds, i - 1, lb_tol);
        }
        // BLACK phase
    ...
    for (unsigned int idx { 0 }; idx < g_ndomains; ++idx) {
      ...
      if (local) {
        ...
        if (offset_ncells[d][idx] < 0) {
          // domain is shrinking -> right boundary moves left, need to send data
          // to the right neighbor ToDo: define fields
          ndfield_t<D, 6> new_fields {... };
          ndfield_t<D, 6> send_fields {... };

          if constexpr (D == Dim::_1D) {
            Kokkos::deep_copy(new_fields,
                              Kokkos::slice(old_fields, { 0, nxnew[0] }));
            Kokkos::deep_copy(send_fields,
                              Kokkos::slice(old_fields, { nxnew[0], nxold[0] }));
          } else if constexpr (D == Dim::_2D) {
            Kokkos::deep_copy(
              new_fields,
              Kokkos::slice(old_fields, { 0, nxnew[0] }, { 0, nxnew[1] }));
            Kokkos::deep_copy(send_fields,
                              Kokkos::slice(old_fields,
                                            { nxnew[0], nxold[0] },
                                            { nxnew[1], nxold[1] }));
          ...
          if constexpr (D == Dim::_1D) {
            Kokkos::deep_copy(
              Kokkos::slice(new_fields, { nxnew[0] - nxold[0], nxnew[0] }),
              old_fields);
          } else if constexpr (D == Dim::_2D) {
            Kokkos::deep_copy(Kokkos::slice(new_fields,
                                            { nxnew[0] - nxold[0], nxnew[0] },
                                            { nxnew[1] - nxold[1], nxnew[1] }),
                              old_fields);
          } else if constexpr (D == Dim::_3D) {
            Kokkos::deep_copy(Kokkos::slice(new_fields,
                                            { nxnew[0] - nxold[0], nxnew[0] },
                                            { nxnew[1] - nxold[1], nxnew[1] },
                                            { nxnew[2] - nxold[2], nxnew[2] }),
                              old_fields);
          }

          MPI_RECV(recv_fields, ...);
          Kokkos::deep_copy(Kokkos::slice(new_fields, { 0, nxnew[0] - nxold[0] }),
                            recv_fields);
        }
      }
    ...
    for (size_t s_idx = 0; s_idx < g_species_params.size(); ++s_idx) {
      ...
      // re-tag any particle that is now out of the new domain bounds.
      {
        ...
        Kokkos::parallel_for(
          "ResetTags_LB",
          sp.rangeActiveParticles(),
          KOKKOS_LAMBDA(int p) { tag_view(p) = ParticleTag::alive; });
      }
...
#if defined(MPI_ENABLED)
              tag_view(
                p) = mpi::SendTag(tag_view(p), i1_view(p) < 0, i1_view(p) >= ni1);
#endif
            });
        }
      } else if constexpr (D == Dim::_2D) {
