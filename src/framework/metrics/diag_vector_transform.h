...
v_Cov2Cart(const coord_t<D>& xi, const vec_t<Dimension::THREE_D>& vi_cov, vec_t<Dimension::THREE_D>& vi_cart) const {
  vi_cart[0] = vi_cov[0] / math::sqrt(h_11(xi));
  vi_cart[1] = vi_cov[1] / math::sqrt(h_22(xi));
  vi_cart[2] = vi_cov[2] / math::sqrt(h_33(xi));
}
/**
...
v_Cart2Cov(const coord_t<D>& xi, const vec_t<Dimension::THREE_D>& vi_cart, vec_t<Dimension::THREE_D>& vi_cov) const {
  vi_cov[0] = vi_cart[0] * math::sqrt(h_11(xi));
  vi_cov[1] = vi_cart[1] * math::sqrt(h_22(xi));
  vi_cov[2] = vi_cart[2] * math::sqrt(h_33(xi));
}
/**
