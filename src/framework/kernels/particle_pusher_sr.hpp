...
namespace ntt {
  ...
  public:
    PusherBase_kernel(const ndfield_t<D, 6>&    EB,
      ...
      is_periodic_i1min = (boundaries[0][0] == BoundaryCondition::PERIODIC);
      is_periodic_i1max = (boundaries[0][1] == BoundaryCondition::PERIODIC);
      if constexpr ((D == Dim2) || (D == Dim3)) {
        ...
        is_periodic_i2min = (boundaries[1][0] == BoundaryCondition::PERIODIC);
        is_periodic_i2max = (boundaries[1][1] == BoundaryCondition::PERIODIC);
      ...
      if constexpr (D == Dim3) {
        ...
        is_absorb_i3min = (boundaries[2][0] == BoundaryCondition::OPEN) ||
                          (boundaries[2][0] == BoundaryCondition::CUSTOM) ||
                          (boundaries[2][0] == BoundaryCondition::ABSORB);
        is_absorb_i3max = (boundaries[2][1] == BoundaryCondition::OPEN) ||
                          (boundaries[2][1] == BoundaryCondition::CUSTOM) ||
                          (boundaries[2][1] == BoundaryCondition::ABSORB);
        is_periodic_i3min = (boundaries[2][0] == BoundaryCondition::PERIODIC);
        is_periodic_i3max = (boundaries[2][1] == BoundaryCondition::PERIODIC);
      }
    }
