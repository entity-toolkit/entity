...
namespace ntt {
  namespace {
    auto InterpretInputForFieldOutput_helper(
      const FieldID&                       fid,
    ...
    auto InterpretInputForFieldOutput_getcomponents(
      const std::vector<std::string>& comps) -> std::vector<std::vector<int>> {
    ...
    auto InterpretInput_getspecies(const std::string& input_quantities)
      -> std::vector<int> {
      std::vector<int> species;
      if (input_quantities.find("_") < input_quantities.size()) {
        auto species_str = SplitString(
          input_quantities.substr(input_quantities.find("_") + 1),
          "_");
  ...
  auto InterpretInputForFieldOutput(const std::string& fld) -> OutputField {
    ...
    auto is_moment = (id == FieldID::T || id == FieldID::Rho ||
                      id == FieldID::Nppc || id == FieldID::N ||
                      id == FieldID::Charge);
    ...
    if (id == FieldID::T) {
      comps = InterpretInputForFieldOutput_getcomponents(
        { fld.substr(1, 1), fld.substr(2, 1) });
    }
