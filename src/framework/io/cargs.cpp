...
namespace ntt {
  ...
  auto CommandLineArguments::getArgument(std::string_view key, std::string_view def) -> std::string_view {
    if (!_initialized) { throw std::runtime_error("# Error: command line arguments have not been parsed."); }
  ...
  auto CommandLineArguments::getArgument(std::string_view key) -> std::string_view {
    if (!this->isSpecified(key)) { throw std::runtime_error("# Error: unspecified key in command line args."); }
