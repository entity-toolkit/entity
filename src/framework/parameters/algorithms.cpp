  ...
  namespace params {
    ...
    void Algorithms::read(real_t                             dx0,
      ...
      fieldsolver_enable = toml::find_or(toml_data,
                                         ...
                                         "enable",
                                         true);
      fieldsolver_stencil_coeffs.emplace();
    ...
    void Algorithms::setParams(const std::map<std::string, bool>& extra,
      ...
      params->set("algorithms.timestep.CFL", CFL.value());
      params->set("algorithms.timestep.dt", dt.value());
      params->set("algorithms.timestep.correction", dt_correction_factor.value());

      params->set("algorithms.current_filters", number_of_current_filters.value());
      ...
      if (extra.at("gr")) {
        params->set("algorithms.gr.pusher_eps", gr_pusher_eps.value());
        params->set("algorithms.gr.pusher_niter", gr_pusher_niter.value());
      }

      if (extra.at("gca")) {
        params->set("algorithms.gca.e_ovr_b_max", gca_e_ovr_b_max.value());
        params->set("algorithms.gca.larmor_max", gca_larmor_max.value());
      }
    }
