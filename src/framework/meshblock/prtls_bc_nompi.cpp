...
namespace ntt {
  template <>
  auto Particles<Dim1, PICEngine>::BoundaryConditions(const Mesh<Dim1>& mesh)
    -> void {
    ...
    Kokkos::parallel_for(
      ...
      ClassLambda(index_t p) {
        i1(p) += ni1 * static_cast<int>(i1(p) < 0) -
                 ni1 * static_cast<int>(i1(p) >= (int)ni1);
      });
  }
  #ifdef MINKOWSKI_METRIC
  template <>
  auto Particles<Dim2, PICEngine>::BoundaryConditions(const Mesh<Dim2>& mesh)
    -> void {
    for (auto& bcs : mesh.boundaries) {
      for (auto& bc : bcs) {
        NTTHostErrorIf((bc != BoundaryCondition::PERIODIC),
    ...
    Kokkos::parallel_for(
      ...
      ClassLambda(index_t p) {
        i1(p) += ni1 * static_cast<int>(i1(p) < 0) -
                 ni1 * static_cast<int>(i1(p) >= (int)ni1);
        i2(p) += ni2 * static_cast<int>(i2(p) < 0) -
                 ni2 * static_cast<int>(i2(p) >= (int)ni2);
      });
  }

  template <>
  auto Particles<Dim3, PICEngine>::BoundaryConditions(const Mesh<Dim3>& mesh)
    -> void {
    for (auto& bcs : mesh.boundaries) {
      for (auto& bc : bcs) {
        NTTHostErrorIf((bc != BoundaryCondition::PERIODIC),
    ...
    Kokkos::parallel_for(
      ...
      ClassLambda(index_t p) {
        ...
        i2(p) += ni2 * static_cast<int>(i2(p) < 0) -
                 ni2 * static_cast<int>(i2(p) >= (int)ni2);
        i3(p) += ni3 * static_cast<int>(i3(p) < 0) -
                 ni3 * static_cast<int>(i3(p) >= (int)ni3);
      });
  }
  ...
  auto Particles<Dim2, PICEngine>::BoundaryConditions(const Mesh<Dim2>& mesh)
    -> void {
    NTTHostErrorIf(
      (mesh.boundaries[0][0] != BoundaryCondition::OPEN) &&
    ...
    NTTHostErrorIf((mesh.boundaries[0][1] != BoundaryCondition::OPEN) &&
                   ...
                   "2D non-Minkowski SR must have open or custom or absorb "
                   "boundaries in x1_max");
  ...
  template <>
  auto Particles<Dim2, GRPICEngine>::BoundaryConditions(const Mesh<Dim2>& mesh)
    -> void {
    ...
    i1h                 -= buffer_h;
    if (i1h < 0) {
      NTTWarn("not enough buffer at rmin below the horizon, setting i1 of "
              "horizon to 0");
    }
    i1h = std::max(i1h, 0);
      ...
      ClassLambda(index_t p) {
        if (tag(p) == ParticleTag::alive) {
          if ((i1(p) < i1h) || (i1(p) >= ni1)) {
            ...
            ux2(p) = ((i2(p) < 1) ? ONE : -ONE) * math::fabs(ux2(p));
          }
        }
      });
  }
  ...
  template <>
  auto Particles<Dim3, GRPICEngine>::BoundaryConditions(const Mesh<Dim3>&)
    -> void {
    NTTHostError("not implemented");
  }

  template <>
  auto Particles<Dim1, SANDBOXEngine>::BoundaryConditions(const Mesh<Dim1>&)
    -> void {
    NTTHostError("not implemented");
  }

  template <>
  auto Particles<Dim2, SANDBOXEngine>::BoundaryConditions(const Mesh<Dim2>&)
    -> void {
    NTTHostError("not implemented");
  }

  template <>
  auto Particles<Dim3, SANDBOXEngine>::BoundaryConditions(const Mesh<Dim3>&)
    -> void {
    NTTHostError("not implemented");
  }
