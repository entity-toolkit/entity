...
namespace ntt {
  ...
  template <>
  auto CreateRangePolicyOnHost<Dim1>(const tuple_t<int, Dim1>& i1,
                                     const tuple_t<int, Dim1>& i2) -> range_h_t<Dim1> {
    index_t i1min = i1[0];
    index_t i1max = i2[0];
  ...
  template <>
  auto CreateRangePolicyOnHost<Dim2>(const tuple_t<int, Dim2>& i1,
                                     const tuple_t<int, Dim2>& i2) -> range_h_t<Dim2> {
    index_t i1min = i1[0];
    index_t i1max = i2[0];
    index_t i2min = i1[1];
    index_t i2max = i2[1];
  ...
  template <>
  auto CreateRangePolicyOnHost<Dim3>(const tuple_t<int, Dim3>& i1,
                                     const tuple_t<int, Dim3>& i2) -> range_h_t<Dim3> {
    index_t i1min = i1[0];
    index_t i1max = i2[0];
    index_t i2min = i1[1];
    index_t i2max = i2[1];
    index_t i3min = i1[2];
    index_t i3max = i2[2];
    return Kokkos::MDRangePolicy<Kokkos::Rank<3>, HostExeSpace>({i1min, i2min, i3min},
                                                                {i1max, i2max, i3max});
  }
